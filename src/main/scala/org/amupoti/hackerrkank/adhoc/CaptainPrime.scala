...
*/
  ...
  def computeFate(persons: List[String]): List[String] = {
    persons map computePosition
  }

  def allLeftPrimes(p: String): Boolean = {

    1 until p.length forall (c => isStringPrime(p.substring(c)))

  }

  def allRightPrimes(p: String): Boolean = {

    1 until p.length forall (c => isStringPrime(p.substring(0, p.length - c)))
  ...
  def isStringPrime(s: String): Boolean = {
    isPrime(s.toInt)
  }
}
